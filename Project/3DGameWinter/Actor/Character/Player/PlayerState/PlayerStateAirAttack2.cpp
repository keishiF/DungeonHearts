...
{
	// アニメーションの再生速度
	constexpr float kAirAtk2AnimSpeed = 1.75f;

	// アニメーション名
	const char* kAirAtk2AnimName = "Player|AirAttack2";
...
void PlayerStateAirAttack2::OnEntry()
{
...
void PlayerStateAirAttack2::OnUpdate(std::shared_ptr<Camera> camera)
{
	auto player = m_parent.lock();
	if (!player) return;
	...
	if (currentFrame >= totalTime)
	{
		if (m_isCombo)
		{
			m_machine.lock()->ChangeState(std::make_shared<PlayerStateAirAttack3>());
		}
...
void PlayerStateAirAttack2::OnLeave()
{
	auto player = m_parent.lock();
	if (!player) return;
	player->GetRigidbody().SetGravity(true);
	printf("プレイヤー：空中攻撃2段目状態リーブ\n");
}
