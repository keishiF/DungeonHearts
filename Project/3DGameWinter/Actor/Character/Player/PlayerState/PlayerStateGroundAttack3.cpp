...
{
	// アニメーションの再生速度
	constexpr float kGroundAtk3AnimSpeed = 1.75f;

	// アニメーション名
	const char* kGroundAtk3AnimName = "Player|GroundAttack3";
...
void PlayerStateGroundAttack3::OnEntry()
{
	auto player = m_parent.lock();
	if (!player) return;
	printf("プレイヤー：地上攻撃3段目状態エントリー\n");
	m_atkSE = LoadSoundMem("Data/Sound/SE/PlayerAtkSE.mp3");
	assert(m_atkSE >= 0);
	player->GetAnimator().ChangeAnim(kGroundAtk3AnimName, kGroundAtk3AnimSpeed, false);
}

void PlayerStateGroundAttack3::OnUpdate(std::shared_ptr<Camera> camera)
{
	auto player = m_parent.lock();
	if (!player) return;
...
void PlayerStateGroundAttack3::OnLeave()
{
	printf("プレイヤー：地上攻撃3段目状態リーブ\n");
}
