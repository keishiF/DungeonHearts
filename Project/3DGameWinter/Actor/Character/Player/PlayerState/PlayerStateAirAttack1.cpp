...
#include "PlayerStateAirAttack2.h"
...
{
	// アニメーションの再生速度
	constexpr float kAirAtk1AnimSpeed = 1.75f;

	// アニメーション名
	const char* kAirAtk1AnimName = "Player|AirAttack1";
...
void PlayerStateAirAttack1::OnEntry()
{
	auto player = m_parent.lock();
	if (!player) return;
	printf("プレイヤー：空中攻撃1段目状態エントリー\n");
	...
	player->GetAnimator().ChangeAnim(kAirAtk1AnimName, kAirAtk1AnimSpeed, false);
}

void PlayerStateAirAttack1::OnUpdate(std::shared_ptr<Camera> camera)
{
	auto player = m_parent.lock();
	if (!player) return;
	...
	constexpr float kAttackEndFrame = 48.0f;
	...
	if (currentFrame >= totalTime)
	{
		if (m_isCombo)
		{
			m_machine.lock()->ChangeState(std::make_shared<PlayerStateAirAttack2>());
		}
...
void PlayerStateAirAttack1::OnLeave()
{
	auto player = m_parent.lock();
	if (!player) return;
	player->GetRigidbody().SetGravity(true);
	printf("プレイヤー：空中攻撃1段目状態リーブ\n");
}
