...
namespace
{
	...
	// ボス時にさらに引く量
	constexpr float kBossExtraBack = -200.0f;
...
void Camera::Update(std::shared_ptr<Player> player, std::shared_ptr<ActorController> actorController)
{
	...
	if (player->IsLockOn() && player->GetLockedOnEnemy())
	{
		...
		if (enemy)
		{
			...
			if (enemy->IsBoss())
			{
				...
				// ボスならカメラをさらに引く
				bossBackOffsetZ = kBossExtraBack;
			}
	...
	// 初期オフセット（プレイヤーの後方、上空）
	Vector3 offset = { kCameraPosX, kCameraPosY, kCameraPosZ + bossBackOffsetZ };
