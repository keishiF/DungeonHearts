...
{
	// アニメーションの再生速度
	constexpr float kGroundAtk1AnimSpeed = 1.75f;

	// アニメーション名
	const char* kGroundAtk1AnimName = "Player|GroundAttack1";
...
void PlayerStateGroundAttack1::OnEntry()
{
...
void PlayerStateGroundAttack1::OnUpdate(std::shared_ptr<Camera> camera)
{
	auto player = m_parent.lock();
	if (!player) return;
	...
	const float animTransFrame = m_parent.lock()->GetAnimator().GetAnimTransitionFrame();
...
void PlayerStateGroundAttack1::OnLeave()
{
	printf("プレイヤー：地上攻撃1段目状態リーブ\n");
}
